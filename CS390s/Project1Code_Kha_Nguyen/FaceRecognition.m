...
%%%%%%%%%%%%%%%% enhancement %%%%%%%%%%%%%%%%%%%%%%
 %im_pro =imread('./Dataset/enrolling/*.bmp');
 %im_en=histeq(im_pro);
 %Did not know how to image enhance properly

 
 %%%%%%%%%%%%%%%%%%%%% TRAINING %%%%%%%%%%%%%%%%%%%%
imlist=dir('./Project1Code_Kha_Nguyen/Dataset/enrolling/*.bmp'); 
im =imread(['./Project1Code_Kha_Nguyen/Dataset/enrolling/',imlist(1).name]);
%img =rgb2gray(im); % converts color img to gray
[r,c]=size(im); 
num_im=length(imlist);
num_p=num_im/5; %fFive image for each person
x=zeros((r*c),num_p);
im_vector=zeros(r*c,num_im);
Mec=zeros(r*c,1);
index=zeros;
index2=zeros;
match=zeros(1,30); %Changed from 30
match2=zeros(1,30);
cmc=zeros(0,30);  %Change for the number of images you want to use
cmc2=zeros(1,30); %Change for the number of images you want to use

%%%%%% convert all images to vector %%%%%%
for i=1:num_im
    im =imread(['./Project1Code_Kha_Nguyen/Dataset/enrolling/',imlist(i).name]);
...
plot(wta(:,2));title('Weights representing Faces of Person1');
figure,plot(wta(:,4));title('Weights representing Faces of Person2');
figure,plot(wta(:,6));title('Weights representing Faces of Person3');
figure,plot(wta(:,8));title('Weights representing Faces of Person4');
figure,plot(wta(:,10));title('Weights representing Faces of Person5');
figure,plot(wta(:,12));title('Weights representing Faces of Person6');
figure,plot(wta(:,14));title('Weights representing Faces of Person7');
figure,plot(wta(:,16));title('Weights representing Faces of Person8');
figure,plot(wta(:,18));title('Weights representing Faces of Person9');
figure,plot(wta(:,20));title('Weights representing Faces of Person10');
...
%%%%%%%%%%%%%%%%%%%%%%%  TESTING  %%%%%%%%%%%%%%%%%%%%%%%%
imlist2=dir('./Project1Code_Kha_Nguyen/Dataset/testing/*.bmp');
num_imt=length(imlist2);
imt_vector=zeros(r*c,num_imt);

%%%%%% convert all test images to vector %%%%%%
for i=1:num_imt
im =imread(['./Project1Code_Kha_Nguyen/Dataset/testing/',imlist2(i).name]);
...
for i=1:30  %% if show CMC of the 1st to 10th rank matching number 
