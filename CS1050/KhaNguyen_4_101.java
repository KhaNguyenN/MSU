...
public class KhaNguyen_4_101 {
   ...
   public static void main(String[ ] args) throws IOException {
      ...
      while (input.hasNext() && (nRead < empl.length)) {
         ...
         empl[nRead].grossPay = calculateGrossPay(empl[nRead].hoursWorked , empl[nRead].payRate);
         empl[nRead].iraAmount = calculateIraInvestmentAount(empl[nRead].grossPay,iraRate);
         empl[nRead].adjustedGrossPay = calculateAdjustedGrossPay(empl[nRead].grossPay,empl[nRead].iraAmount);
         empl[nRead].taxAmount = calculateTaxes(empl[nRead].adjustedGrossPay,stateHolding,fedHolding);
         empl[nRead].netPay = calculateNetPay(empl[nRead].adjustedGrossPay,empl[nRead].taxAmount);
         empl[nRead].savingsAmount = calculateSavingAmount(empl[nRead].netPay,savingsRate);
         nRead++;
      } // End while
   ...
   public static double calculateGrossPay( double hoursWorked, double payRate){
      ...
      if(hoursWorked <= 40.0){
         ...
         double hoursGotPaidOneHalf = 50 - 40.0;
         double hoursGotPaidDouble = hoursWorked - 50.0;
         grossPay = (payRate * 40.0) + ((payRate*1.5) * hoursGotPaidOneHalf) + ( (payRate * 2) * hoursGotPaidDouble);
      }
...
//***************************************************************************************
   ...
   public static void printHeading(String printOrder){
      final String DOLLAR = "#,##0.00";
      tools.outputLine("\nPrint in " + printOrder.toLowerCase() + " order\n");
      tools.outputLine("                   Mobile Apps Galore, Inc. - Payroll Report          \n");
      tools.outputLine(        tools.padString("Name",20) +
      ...
      tools.outputLine(tools.padString("",10,"-","-") +
              "   " + tools.padString("",5,"-","-") +
              "   " + tools.padString("",5,"-","-") +
              "   " + tools.padString("",5,"-","-") +
              "   " + tools.padString("",5,"-","-") +
...
//***************************************************************************************   
   ...
   public static void printDetailLine(Employee[] empl,int nEntries){
      final String DOLLAR = "#,##0.00";
      for (int i = 0; i < nEntries; i++) {
         tools.outputLine(        tools.padString(empl[i].name,20) +
                 "   " + tools.leftPad(empl[i].grossPay, 10, DOLLAR) +
                 "   " + tools.leftPad(empl[i].netPay, 10, DOLLAR) +
                 "   " + tools.leftPad(empl[i].savingsAmount + empl[i].iraAmount, 10, DOLLAR) +
